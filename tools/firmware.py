...
def main(argv=None):
    ...
    if argv[:1] in (["logger"], ["logger-cli"]):
        ...
        # Share the page listener so SSH users only need one forwarded port.
        defaults = (["--dashboard-channel", "1", "--log-channel", "0", "--http-host", "127.0.0.1", "--http-port", "8080"] if mode == "logger" else ["--mode", "dashboard", "--channel", "1", "--log-channel", "0", "--connect", "normal", "--poll-ms", "20", "--refresh-ms", "100"])

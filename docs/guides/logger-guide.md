...
v11 在 v10 数据后附带 pitch 的命令单位/状态/原始命令、实际电流反馈、内外环速度、
P/I/D/总输出、PID 增益、输出限幅和内环 dt。原始命令与电流仍是协议刻度，不是安培或伏特；
无效或控制未运行时浮点字段为 `nan`。网页“All”页的“Pitch PID Diagnostics”面板和主机
logger 文件会保存这些字段，可用来区分输出不足与机械阻力。旧 v3～v10 帧仍可解析。
